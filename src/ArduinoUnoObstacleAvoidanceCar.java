public class ArduinoUnoObstacleAvoidanceCar {
    public static void main(String[] args) {
        System.out.println(" This is a C++ code for Obstacle avoidance car using Arduino uno ");

        /*

upload code via arduino uno IDE TO Arduino Uno board
...
int motorOffset = 10;  // change it to 20   //Factor to account for one side being more powerful
...
void loop() {
  ...
  {
    case 0:           //Turn left
      turnLeft(-90);  // 100   try -90  , 90  180 ,  -180
      break;
    case 1:           //Turn around
      turnLeft(180);  // 100
      break;
...
{
  rightBack.run(FORWARD);  // add different configuation to move forward backward lef right
  // delay (100);
  rightFront.run(FORWARD);  // use void moveLEFT  AND
  // delay (100);
  leftFront.run(FORWARD);  // VOID MOVE RIGHT()    INSTADE OF    void accelaration and viod disacceleration()
  // delay (100);
...
void turnLeft(int duration)  //Set motors to turn left for the specified duration then stop
{
  ...
  delay(duration);                 // set as duration bt default
  rightBack.setSpeed(motorSpeed);  //Set the motors to the motor speed
  rightFront.setSpeed(motorSpeed);
  leftFront.setSpeed(motorSpeed + motorOffset);
  leftBack.setSpeed(motorSpeed + motorOffset);
...
void turnRight(int duration)  //Set motors to turn right for the specified duration then stop
{
  ...
  leftFront.setSpeed(motorSpeed + motorOffset + turnSpeed);
  leftBack.setSpeed(motorSpeed + motorOffset + turnSpeed);
  rightBack.run(BACKWARD);  // ude dealy(100), instade by removing aboe code from righrBack.setSpeed(motorSpeed+turnSpeed);
